...
function getRegistros(orderby, sentido, registros, pagina, busqueda, objeto) {
    if (!requestSent) {
        requestSent = true;
        var parametros = {
            funcion: "getRegistrosFiltroOpciones",
            orderby: orderby,
            sentido: sentido,
            registros: registros,
            pagina: pagina,
...
$("#guardarDatosProductoAlt").submit(function (event) {
    if (!requestSent) {
        requestSent = true;
        var parametros = {
            funcion: "addProductoAlt",
            descripcion: $("#descripcionAddAlt").val(),
            opcion: codigo
        }
        $.ajax({
            type: "POST",
...
$("#actualidarDatosProductoAlt").submit(function (event) {
    if (!requestSent) {
        requestSent = true;
        var parametros = {
            funcion: "updateProductoAlt",
            codigo: codigo,
            descripcion: $("#descripcionUpdateAlt").val()
        }
        $.ajax({
            type: "POST",
            url: 'controller/productos.controller.php',
