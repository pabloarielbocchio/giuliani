...
<div class="row" style="margin-top: -10px; font-size: 11px;" >
    ...
    <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12" style="float: left;text-align: left;margin-top: 0px; display: none;">
        <label style="text-align: right; margin-right: 10px;">Elija un<br />Área:</label>
        <select id="select_area" class="form-control asistencia" style="width: 65%;display: inline-block;vertical-align: middle; font-size: 11px; height: auto; margin-top: -15px;">
            <?php 
                foreach ($destinos_rol as $dest){
                    if ($dest["destino_id"] == $_SESSION["area"]){
                        echo '<option value="' . $dest["destino_id"] . '" selected>' . $dest["descripcion"] . '</option>';
                    } else {
                        echo '<option value="' . $dest["destino_id"] . '" >' . $dest["descripcion"] . '</option>';
                    }
                }
            ?>
        </select>
