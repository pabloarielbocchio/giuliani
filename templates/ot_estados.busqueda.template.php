<table id="tabla" namefile="Ot_estados" totales="<?php echo $_SESSION["totales"]; ?>" registros="<?php echo $_SESSION['cant_reg']; ?>" pagina="<?php $_SESSION['pagina']; ?>" class="table table-striped table-hover" mes="<?php echo $mes; ?>" anio="<?php echo $anio; ?>" dia="<?php echo $dia; ?>" opcion="<?php echo $opcion; ?>"> 
    <thead>
        <tr class="row " style="background-color: transparent;">
            ...
            <th class="text-center ordena" orderby="sector" sentido="asc">Estado</th>
        ...
        <?php foreach ($registros as $usu) { ?>
            <tr class="row" codigo="<?php echo $usu["codigo"]; ?>">
                ...
                <td class="text-center noExl" style="vertical-align: middle;">
                    <div class="editOt_estado" style="float: left; margin-left: 10px;"><a href="#"><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span></a></div>
                    <div class="deleteOt_estado" style="float: right;margin-right: 10px;"><a href="#"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></a></div>
                </td>
            </tr>
<?php } ?>
    </tbody>
...
<script>
    ...
    $("#btn-eliminar-ot_estado").click(function () {
        if (!requestSent) {
            requestSent = true;
            var parametros = {
                funcion: "deleteOt_estado",
                codigo: codigo
            }
            $.ajax({
                type: "POST",
    ...
    $(".editOt_estado").click(function () {
        codigo = $(this).closest('tr').attr("codigo");
        $("#text-header-body").html("¿Desea eliminar el registro?");
        $("#btn-eliminar-ot_estado").css("display", "inline-block");
        $("#btn-cancelar").text("Cancelar");

        var parametros = {
            funcion: "getOt_estado",
            codigo: codigo
        }
        $.ajax({
            type: "POST",
            ...
            success: function (data) {
                ...
                $('#produccionUpdate').val(datos.ot_prod_id);
    ...
    $(".deleteOt_estado").click(function () {
        codigo = $(this).closest('tr').attr("codigo");
