<table id="tabla" namefile="Productos" totales="<?php echo $_SESSION["totales"]; ?>" registros="<?php echo $_SESSION['cant_reg']; ?>" pagina="<?php $_SESSION['pagina']; ?>" class="table table-striped table-hover" mes="<?php echo $mes; ?>" anio="<?php echo $anio; ?>" dia="<?php echo $dia; ?>" opcion="<?php echo $opcion; ?>"> 
    <thead>
        <tr class="row " style="background-color: transparent;">
            <th class="col-lg-6 col-md-6 col-sm-6 col-xs-6 text-left ordena" orderby="prod_f_id" sentido="asc">Producto Nivel 6</th>
            <th class="col-lg-5 col-md-5 col-sm-5 col-xs-5 text-left ordena" orderby="prod_standar_id" sentido="asc">Producto Standard</th>
    ...
    <tbody id="body">
        <?php foreach ($registros as $usu) { ?>
            <tr class="row" codigo="<?php echo $usu["codigo"]; ?>">
                <td class="col-lg-6 col-md-6 col-sm-6 col-xs-6 text-left" style="vertical-align: middle;">
                    <?php 
                        foreach ($prod_f as $r){
                            if ($r["codigo"] == $usu["prod_f_id"]){
                                echo $r["descripcion"];
                            }
                        }
                    ?>
                ...
                <td class="col-lg-5 col-md-5 col-sm-5 col-xs-5 text-left" style="vertical-align: middle;">
                    <?php 
                        foreach ($prod_s as $r){
                            if ($r["codigo"] == $usu["prod_standar_id"]){
                                echo $r["descripcion"];
                            }
                        }
                    ?>
...
<script>
    ...
    $("#btn-eliminar-producto").click(function () {
        if (!requestSent) {
            ...
            $.ajax({
                type: "POST",
                url: 'controller/productos_x.controller.php',
                data: parametros,
    ...
    $(".editProducto").click(function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#prodfUpdate').val(datos.prod_f_id);
                $('#prodsUpdate').val(datos.prod_standar_id);
            },
            error: function () {
                alert("Error");
            }
