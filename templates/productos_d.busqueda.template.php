<table id="tabla" namefile="Productos" totales="<?php echo $_SESSION["totales"]; ?>" registros="<?php echo $_SESSION['cant_reg']; ?>" pagina="<?php $_SESSION['pagina']; ?>" class="table table-striped table-hover" mes="<?php echo $mes; ?>" anio="<?php echo $anio; ?>" dia="<?php echo $dia; ?>" opcion="<?php echo $opcion; ?>"> 
    <thead>
        <tr class="row " style="background-color: transparent;">
            ...
            <th class="col-lg-4 col-md-4 col-sm-4 col-xs-4 text-left ordena" orderby="cod_prod_nb" sentido="asc">Producto Nivel 3</th>
    ...
    <tbody id="body">
        <?php foreach ($registros as $usu) { ?>
            <tr class="row" codigo="<?php echo $usu["codigo"]; ?>">
                ...
                <td class="col-lg-4 col-md-4 col-sm-4 col-xs-4 text-left" style="vertical-align: middle;">
                    <?php 
                        foreach ($prod_c as $r){
                            if ($r["codigo"] == $usu["cod_prod_nc"]){
                                echo $r["descripcion"];
                            }
                        }
                    ?>
...
<div class="col-lg-4" style="float: left;text-align: left;margin-top: 8px; display: none;" id='leyenda_paginacion_aux'>
...
<script>
    ...
    $("#btn-eliminar-producto").click(function () {
        if (!requestSent) {
            ...
            $.ajax({
                type: "POST",
                url: 'controller/productos_d.controller.php',
                data: parametros,
