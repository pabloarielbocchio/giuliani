<table id="tabla" namefile="Prioridades" totales="<?php echo $_SESSION["totales"]; ?>" registros="<?php echo $_SESSION['cant_reg']; ?>" pagina="<?php $_SESSION['pagina']; ?>" class="table table-striped table-hover" mes="<?php echo $mes; ?>" anio="<?php echo $anio; ?>" dia="<?php echo $dia; ?>" opcion="<?php echo $opcion; ?>"> 
    <thead>
        <tr class="row " style="background-color: transparent;">
            ...
            <th class="col-lg-1 col-md-1 col-sm-1 col-xs-1 text-left ordena" orderby="descripcion" sentido="asc">Prioridad</th>
    ...
    <tbody id="body">
        <?php foreach ($registros as $usu) { ?>
            <tr class="row" codigo="<?php echo $usu["codigo"]; ?>">
                ...
                <td class="col-lg-1 col-md-1 col-sm-1 col-xs-1 text-left" style="vertical-align: middle;"><?php echo $usu["prioridad"]; ?></td>
                <td class="col-lg-1 col-md-1 col-sm-1 col-xs-1 text-center noExl" style="vertical-align: middle;">
                    <div class="editPrioridad" style="float: left; margin-left: 10px;"><a href="#"><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span></a></div>
                    <div class="deletePrioridad" style="float: right;margin-right: 10px;"><a href="#"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></a></div>
                </td>
            </tr>
<?php } ?>
    </tbody>
...
<script>
    ...
    $("#btn-eliminar-prioridad").click(function () {
        if (!requestSent) {
            requestSent = true;
            var parametros = {
                funcion: "deletePrioridad",
                codigo: codigo
            }
            $.ajax({
                type: "POST",
                url: 'controller/prioridades.controller.php',
                data: parametros,
    ...
    $(".editPrioridad").click(function () {
        codigo = $(this).closest('tr').attr("codigo");
        $("#text-header-body").html("¿Desea eliminar el registro?");
        $("#btn-eliminar-prioridad").css("display", "inline-block");
    ...
    $(".deletePrioridad").click(function () {
        codigo = $(this).closest('tr').attr("codigo");
