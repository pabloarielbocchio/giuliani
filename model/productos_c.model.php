...
class ProductosModel {  
    ...
    public function getCountProductos(){
        try {
            $sql = "SELECT count(*) FROM productos_nivel_c;";
    ...
    public function getRegistrosFiltro($orderby, $sentido, $registros, $pagina, $busqueda){
        try {
            ...
            $sql = "SELECT * FROM productos_nivel_c WHERE descripcion like '%" . $busqueda . "%' ORDER BY " . $orderby . " " . $sentido;
    ...
    public function deleteProducto($codigo){
        try {
            ...
            $stmt = $this->conn->prepare('DELETE from productos_nivel_c where codigo = ? AND ( codigo NOT IN (SELECT ad.cod_prod_nc FROM productos_nivel_d ad) )');
    ...
    public function addProducto($descripcion, $prodb){
        ...
        try {
            ...
            $stmt->bindValue(2, $prodb, PDO::PARAM_INT);
    ...
    public function updateProducto($codigo, $descripcion, $prodb){
        ...
        try {
            $this->conn->beginTransaction();
            $stmt = $this->conn->prepare('UPDATE productos_nivel_c set '
                                            . 'descripcion = ? , '
                                            . 'cod_prod_nb = ? , '
                                            . 'usuario_m = ?, '
                                            . 'fecha_m = ? '
    ...
    public function getProducto($codigo){
        try {
            $sql = "SELECT * FROM productos_nivel_c WHERE codigo = " . $codigo . ";";
            $query = $this->conn->prepare($sql);
            $query->execute();
            if ($query->rowCount() > 0) {
                $result = $query->fetchAll();
    ...
    public function getProductosB(){
        try {
            $sql = "SELECT 
                        pnb.codigo,
                        (select concat(pnb.descripcion, ' - A: ', descripcion) from productos_nivel_a where codigo = pnb.cod_prod_na) as descripcion
                    FROM productos_nivel_b pnb order by pnb.descripcion;";
            $query = $this->conn->prepare($sql);
            $query->execute();
            if ($query->rowCount() > 0) {
                $result = $query->fetchAll();
