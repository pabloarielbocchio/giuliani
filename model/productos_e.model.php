...
class ProductosModel {  
    ...
    public function getCountProductos(){
        try {
            $sql = "SELECT count(*) FROM productos_nivel_e;";
    ...
    public function getRegistrosFiltro($orderby, $sentido, $registros, $pagina, $busqueda){
        try {
            ...
            $sql = "SELECT * FROM productos_nivel_e WHERE descripcion like '%" . $busqueda . "%' ORDER BY " . $orderby . " " . $sentido;
    ...
    public function deleteProducto($codigo){
        try {
            ...
            $stmt = $this->conn->prepare('DELETE from productos_nivel_e where codigo = ? AND ( codigo NOT IN (SELECT ad.cod_prod_ne FROM productos_nivel_f ad) )');
    ...
    public function updateProducto($codigo, $descripcion, $prodd){
        ...
        try {
            $this->conn->beginTransaction();
            $stmt = $this->conn->prepare('UPDATE productos_nivel_e set '
                                            . 'descripcion = ? , '
                                            . 'cod_prod_nd = ? , '
                                            . 'usuario_m = ?, '
                                            . 'fecha_m = ? '
    ...
    public function getProducto($codigo){
        try {
            $sql = "SELECT * FROM productos_nivel_e WHERE codigo = " . $codigo . ";";
            $query = $this->conn->prepare($sql);
            $query->execute();
            if ($query->rowCount() > 0) {
                $result = $query->fetchAll();
