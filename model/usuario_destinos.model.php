...
class Usuario_destinosModel {
    ...
    public function getCountUsuario_destinos(){
        try {
            $sql = "SELECT count(*) FROM usuarios_destinos;";
    ...
    public function getRegistrosFiltro($orderby, $sentido, $registros, $pagina, $busqueda){
        try {
            ...
            $sql = "SELECT * FROM usuarios_destinos WHERE codigo like '%" . $busqueda . "%' ORDER BY " . $orderby . " " . $sentido;
    ...
    public function deleteUsuario_destino($codigo){
        try {
            ...
            $stmt = $this->conn->prepare('DELETE from usuarios_destinos where codigo = ?');
    ...
    public function updateUsuario_destino($codigo, $usuario, $destino){
        ...
        try {
            $this->conn->beginTransaction();
            $stmt = $this->conn->prepare('UPDATE usuarios_destinos set '
                                            . 'usuario_id = ? , '
                                            . 'destino_id = ? , '
                                            . 'usuario_m = ?, '
                                            . 'fecha_m = ? '
    ...
    public function getUsuario_destino($codigo){
        try {
            $sql = "SELECT * FROM usuarios_destinos WHERE codigo = " . $codigo . ";";
            $query = $this->conn->prepare($sql);
            $query->execute();
            if ($query->rowCount() > 0) {
                $result = $query->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function getUsuarios(){
        try {
            $sql = "SELECT codigo, usuario as descripcion FROM usuarios order by usuario;";
            $query = $this->conn->prepare($sql);
            $query->execute();
            if ($query->rowCount() > 0) {
                $result = $query->fetchAll(PDO::FETCH_ASSOC);
