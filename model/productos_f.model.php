...
class ProductosModel {  
    ...
    public function getCountProductos(){
        try {
            $sql = "SELECT count(*) FROM productos_nivel_f;";
    ...
    public function getRegistrosFiltro($orderby, $sentido, $registros, $pagina, $busqueda){
        try {
            ...
            $sql = "SELECT * FROM productos_nivel_f WHERE descripcion like '%" . $busqueda . "%' ORDER BY " . $orderby . " " . $sentido;
    ...
    public function deleteProducto($codigo){
        try {
            ...
            $stmt = $this->conn->prepare('DELETE from productos_nivel_f where codigo = ? AND ( codigo NOT IN (SELECT ad.prod_f_id FROM productos_configuraciones ad) )');
    ...
    public function addProducto($descripcion, $prode){
        ...
        try {
            ...
            $stmt->bindValue(2, $prode, PDO::PARAM_INT);
    ...
    public function updateProducto($codigo, $descripcion, $prode){
        $hoy = date("Y-m-d H:i:s");
        try {
            $this->conn->beginTransaction();
            $stmt = $this->conn->prepare('UPDATE productos_nivel_f set '
                                            . 'descripcion = ? , '
                                            . 'cod_prod_ne = ? , '
                                            . 'usuario_m = ?, '
                                            . 'fecha_m = ? '
    ...
    public function getProducto($codigo){
        try {
            $sql = "SELECT * FROM productos_nivel_f WHERE codigo = " . $codigo . ";";
            $query = $this->conn->prepare($sql);
            $query->execute();
            if ($query->rowCount() > 0) {
                $result = $query->fetchAll();
