...
class TiposModel {  
    ...
    public function getCountTipos(){
        try {
            $sql = "SELECT count(*) FROM orden_trabajos_tipos;";
    ...
    public function getRegistrosFiltro($orderby, $sentido, $registros, $pagina, $busqueda){
        try {
            ...
            $sql = "SELECT * FROM orden_trabajos_tipos WHERE codigo > 0 and nombre like '%" . $busqueda . "%' ORDER BY " . $orderby . " " . $sentido;
    ...
    public function deleteTipo($codigo){
        try {
            ...
            $stmt = $this->conn->prepare('DELETE from orden_trabajos_tipos where codigo = ? and codigo not in (select orden_trabajos_tipo_id from orden_trabajos) ');
    ...
    public function addTipo($descripcion){
        ...
        try {
            $this->conn->beginTransaction();
            $stmt = $this->conn->prepare('INSERT INTO orden_trabajos_tipos (nombre, usuario_m, fecha_m) VALUES (?,?,?);');
    ...
    public function updateTipo($codigo, $descripcion){
        ...
        try {
            $this->conn->beginTransaction();
            $stmt = $this->conn->prepare('UPDATE orden_trabajos_tipos set '
                                            . 'nombre = ? , '
                                            . 'usuario_m = ?, '
                                            . 'fecha_m = ? '
                                            . ' where codigo = ?');            
    ...
    public function getTipo($codigo){
        try {
            $sql = "SELECT * FROM orden_trabajos_tipos WHERE codigo = " . $codigo . ";";
            $query = $this->conn->prepare($sql);
            $query->execute();
            if ($query->rowCount() > 0) {
                $result = $query->fetchAll();
