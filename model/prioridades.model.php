...
class PrioridadesModel {
    ...
    public function getRegistrosFiltro($orderby, $sentido, $registros, $pagina, $busqueda){
        try {
            ...
            $sql = "SELECT * FROM prioridades WHERE descripcion like '%" . $busqueda . "%' ORDER BY " . $orderby . " " . $sentido;
    ...
    public function deletePrioridad($codigo){
        try {
            ...
            $stmt = $this->conn->prepare('DELETE from prioridades where codigo = ?');
    ...
    public function addPrioridad($descripcion, $prioridad){
        ...
        try {
            $this->conn->beginTransaction();
            $stmt = $this->conn->prepare('INSERT INTO prioridades (descripcion, prioridad, usuario_m, fecha_m) VALUES (?,?,?,?);');
            $stmt->bindValue(1, $descripcion, PDO::PARAM_STR);
            $stmt->bindValue(2, $prioridad, PDO::PARAM_INT);
    ...
    public function updatePrioridad($codigo, $descripcion, $prioridad){
        ...
        try {
            $this->conn->beginTransaction();
            $stmt = $this->conn->prepare('UPDATE prioridades set '
                                            . 'descripcion = ? , '
                                            . 'prioridad = ? , '
                                            . 'usuario_m = ?, '
                                            . 'fecha_m = ? '
    ...
    public function getPrioridad($codigo){
        try {
            $sql = "SELECT * FROM prioridades WHERE codigo = " . $codigo . ";";
            $query = $this->conn->prepare($sql);
            $query->execute();
            if ($query->rowCount() > 0) {
                $result = $query->fetchAll();
