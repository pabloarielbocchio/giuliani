...
class ProductosModel {  
    ...
    public function getRegistrosFiltro($orderby, $sentido, $registros, $pagina, $busqueda){
        try {
            ...
            $sql = "SELECT * FROM productos_nivel_a WHERE descripcion like '%" . $busqueda . "%' ORDER BY " . $orderby . " " . $sentido;
    ...
    public function deleteProducto($codigo){
        try {
            ...
            $stmt = $this->conn->prepare('DELETE from productos_nivel_a where codigo = ? AND ( codigo NOT IN (SELECT ad.cod_prod_na FROM productos_nivel_b ad) )');
    ...
    public function updateProducto($codigo, $descripcion){
        ...
        try {
            $this->conn->beginTransaction();
            $stmt = $this->conn->prepare('UPDATE productos_nivel_a set '
                                            . 'descripcion = ? , '
                                            . 'usuario_m = ?, '
                                            . 'fecha_m = ? '
                                            . ' where codigo = ?');            
    ...
    public function getProducto($codigo){
        try {
            $sql = "SELECT * FROM productos_nivel_a WHERE codigo = " . $codigo . ";";
            $query = $this->conn->prepare($sql);
            $query->execute();
            if ($query->rowCount() > 0) {
                $result = $query->fetchAll();
