...
class UnidadesModel {
    ...
    public function deleteUnidad($codigo){
        try {
            ...
            $stmt = $this->conn->prepare('DELETE from unidades where codigo = ? AND ( codigo NOT IN (SELECT ad.unidad_id FROM productos_nivel_d ad) AND codigo NOT IN (SELECT ad.unidad_id FROM productos_personalizados ad) )');
