...
class SeccionesModel {  
    ...
    public function getRegistrosFiltro($orderby, $sentido, $registros, $pagina, $busqueda){
        try {
            ...
            $sql = "SELECT * FROM secciones WHERE descripcion like '%" . $busqueda . "%' ORDER BY " . $orderby . " " . $sentido;
    ...
    public function deleteSeccion($codigo){
        try {
            ...
            $stmt = $this->conn->prepare('DELETE from secciones where codigo = ? AND ( codigo NOT IN (SELECT ad.seccion_id FROM orden_trabajos_detalles ad) )');
    ...
    public function addSeccion($descripcion){
        ...
        try {
            $this->conn->beginTransaction();
            $stmt = $this->conn->prepare('INSERT INTO secciones (descripcion, usuario_m, fecha_m) VALUES (?,?,?);');
    ...
    public function updateSeccion($codigo, $descripcion){
        ...
        try {
            $this->conn->beginTransaction();
            $stmt = $this->conn->prepare('UPDATE secciones set '
                                            . 'descripcion = ? , '
                                            . 'usuario_m = ?, '
                                            . 'fecha_m = ? '
                                            . ' where codigo = ?');            
    ...
    public function getSeccion($codigo){
        try {
            $sql = "SELECT * FROM secciones WHERE codigo = " . $codigo . ";";
            $query = $this->conn->prepare($sql);
            $query->execute();
            if ($query->rowCount() > 0) {
                $result = $query->fetchAll();
